...
open class SearchTextField: UITextField {
  ...
  public let selectionPublisher = PassthroughSubject<SearchTextFieldItem, Never>()
  ...
  public func filter(forceShowAll addAll: Bool) {
    clearResults()
...
extension SearchTextField: UITableViewDelegate, UITableViewDataSource {
  ...
  public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    selectionPublisher.send(filteredResults[indexPath.row])
...
open class SearchTextFieldItem {
  ...
  weak var attachment: AnyObject? {
    willSet {
      guard attachment.isNil else { return }
      fatalError()
    }
  }
  
  public init(title: String, subtitle: String?, image: UIImage?, attachment: AnyObject?) {
    self.title = title
