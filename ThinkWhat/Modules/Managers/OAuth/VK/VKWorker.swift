...
//    }
    ...
    class func accountInfo(completion: @escaping(Result<JSON,Error>)->()) {
        ...
        VK.API.Custom.method(name: "users.get", parameters: [Parameter.userIDs.rawValue: id, Parameter.fields.rawValue: "\(Parameter.firstName.rawValue), \(Parameter.lastName.rawValue), \(Parameter.sex.rawValue), \(Parameter.bdate.rawValue), \(Parameter.photo.rawValue), \(Parameter.domain.rawValue)"])
            .onSuccess {
                var json = JSON($0)
