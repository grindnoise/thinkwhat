...
//
//    var surveyLink: SurveyReference! {
//        didSet {
//            likeButton.removeAllAnimations()
//            title = surveyLink.topic.title//"Опрос №\(surveyLink.ID)"
//            likeButton.state = Array(Surveys.shared.favoriteReferences.keys).filter( {$0.id == surveyLink.id }).isEmpty ? .disabled : .enabled
//        }
//    }
//
//    var survey: Survey? {
...
//                }
//                if let userProfile = survey!.owner as? Userprofile, let image = userProfile.image as? UIImage {
//                    NotificationCenter.default.post(name: Notifications.UI.ImageReceived, object: nil)
//                } else if shouldDownloadImages, let userProfile = survey!.owner as? Userprofile, let url = userProfile.imageURL as? String {
//                    API.shared.downloadImage(url: url) {
//                        image, error in
//                        if error != nil {
...
//            isLoadingImages = !survey!.media.filter({ $0.image != nil }).isEmpty
...
//                    showNavTitle()
//                }
//            } else if let url  = survey?.owner.imageURL?.absoluteString {
//                API.shared.downloadImage(url: url) {
//                    image, error in
//                    if error != nil {
//                        print(error!.localizedDescription)
...
//                    if let color = survey!.topic.parent?.tagColor {
...
//                } else {
//                    let attrString = NSMutableAttributedString()
//                    attrString.append(NSAttributedString(string: "  \(surveyLink.topic.title.uppercased())", attributes: StringAttributes.getAttributes(font: StringAttributes.font(name: StringAttributes.Fonts.Style.Bold, size: 9), foregroundColor: .white, backgroundColor: .clear)))
//                    attrString.append(NSAttributedString(string: " / ", attributes: StringAttributes.getAttributes(font: StringAttributes.font(name: StringAttributes.Fonts.Style.Regular, size: 9), foregroundColor: .white, backgroundColor: .clear)))
//                    attrString.append(NSAttributedString(string: "\((surveyLink.topic.parent?.title.uppercased() ?? ""))  ", attributes: StringAttributes.getAttributes(font: StringAttributes.font(name: StringAttributes.Fonts.Style.Semibold, size: 9), foregroundColor: .white, backgroundColor: .clear)))
//                    _cell.label.text = surveyLink.title
//                    _cell.survey = surveyLink
//                    _cell.label.text = survey!.title
//                    if let color = surveyLink.topic.parent?.tagColor {
//                        _cell.category.backgroundColor = color//.withAlphaComponent(0.5)
//                        _cell.date.backgroundColor = color
//                        _cell.join.backgroundColor = color
//                        _cell.join_2.backgroundColor = color
...
//                cell = _cell
//            } else if indexPath.row == 2, survey!.url != nil, let _cell = tableView.dequeueReusableCell(withIdentifier: "link", for: indexPath) as? SurveyLinkCell {
//                _cell.delegate = self
//                cell = _cell
//            } else if indexPath.row == 3, survey!.url != nil, let _cell = tableView.dequeueReusableCell(withIdentifier: "youtube", for: indexPath) as? SurveyYoutubeCell {
//                let isYoutube = isYoutubeLink(checkString: survey!.url!.absoluteString)
////                _cell.playerView.delegate = self
//                _cell.delegate = self
//                if isYoutube {
//                    _cell.loadVideo(url: survey!.url!)
//                }
//                if isInitialLoading {
//                    _cell.contentView.alpha = 1
//                    let isYoutube = self.isYoutubeLink(checkString: self.survey!.url!.absoluteString)
//                    if isYoutube {
//                        _cell.loadVideo(url: self.survey!.url!)
//                    }
//                }
//                cell = _cell
...
//                if let userProfile = survey?.owner {
...
//                    } else {
//                        API.shared.downloadImage(url: userProfile.imageURL.absoluteString) {
//                            image, error in
//                            if error != nil {
//                                showAlert(type: CustomAlertView.AlertType.Warning, buttons: [["Закрыть": [CustomAlertView.ButtonType.Ok: nil]]], title: "Ошибка", body: "Изображение не было загружено. \(error!.localizedDescription)")
//                            }
...
//                if survey!.url != nil, !isYoutubeLink(checkString: survey!.url!.absoluteString) {
...
//
//        requestAttempt += 1
//        API.shared.loadSurvey(survey: surveyLink, addViewCount: true) {
//            json, error in
//            if error != nil {
...
//            if json != nil {
//                print(json!)
////                if let _survey = Survey(json!.rawValue) {
...
////                        })
//                        mark = true
//                        if Array(Surveys.shared.favoriteReferences.keys).filter( {$0.id == surveyLink.id }).isEmpty { Surveys.shared.favoriteReferences[self.surveyLink] = Date() }
//                    } else {
//                        likeButton.state = .disabled
//                        mark = false
//                        if let key = Surveys.shared.favoriteReferences.keys.filter({ $0.id == surveyLink.id }).first {
...
//                    API.shared.addFavorite(mark: mark, survey: surveyLink!) {
//                        _, error in
...
//        API.shared.postVote(result: result) {
//            json, error in
//            if error != nil {
...
////                                AppData.shared.userProfile.balance += 1//SurveyPoints.Vote.rawValue
//                            }
//                        }
...
//                }
//            } else if (sender as! UIButton).accessibilityIdentifier == "Close", let claimCategory = (sender as! UIButton).layer.value(forKey: "claimCategory") as? Claim {
//                UIView.animate(withDuration: 0.2, delay: 0, options: .curveEaseInOut, animations: {
//                    self.tempClaimButton?.alpha = 0
//                }) {
//                    _ in
...
//                }
//            } else {
////                if let url = URL(string: survey!.url!) {
...
//        let  userImage = UIImageView(frame: CGRect(origin: .zero, size: navTitleSize))
////        userImage.image = survey!.owner!.image!.circularImage(size: navTitleSize, frameColor: K_COLOR_RED)//survey?.userProfile?.image
//        userImage.alpha = 0
//        navTitle = userImage
