...
class CostCollectionView: UICollectionView {
    ...
    private weak var parent: CostView?
    private var source: UICollectionViewDiffableDataSource<Section, CostItem>!
    private var dataItems: [CostItem] {
        return dataProvider?.costItems ?? []
    }
    private var observers: [NSKeyValueObservation] = []

    init(dataProvider: PollCreationControllerOutput?, parent: CostView) {
...
//                separatorCongig.color = .systemRed
//                separatorCongig.bottomSeparatorInsets
    ...
    private func setObservers() {
        observers.append(observe(\CostCollectionView.contentSize, options: .new) { [weak self] (_, change) in
            guard let self = self else { return }
            self.parent?.onChildHeightChange(change.newValue?.height ?? 0)
        })
    }
}
