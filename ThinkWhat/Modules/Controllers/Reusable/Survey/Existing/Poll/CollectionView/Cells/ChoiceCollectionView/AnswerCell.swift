...
private extension AnswerCell {
  ...
  func updateUI(animated: Bool = true) {
    ...
    func animate() {
      UIView.animate(
        ...
        animations: { [weak self] in
          ...
          if survey.isComplete {
            self.imageView.tintColor = Colors.getColor(forId: self.item.order)
      ...
      UIView.transition(with: textView, duration: 0.15, options: .transitionCrossDissolve) { [weak self] in
        ...
        if survey.isComplete {
          self.stackView.backgroundColor = !self.isAnswerSelected ? .clear : self.traitCollection.userInterfaceStyle == .dark ? .tertiarySystemBackground : Colors.getColor(forId: self.item.order).withAlphaComponent(0.15)
        } else {
          self.stackView.backgroundColor = !self.isAnswerSelected ? .clear : self.traitCollection.userInterfaceStyle == .dark ? .tertiarySystemBackground : survey.topic.tagColor.withAlphaComponent(0.15)
        }
      } completion: { _ in }
    }
    ...
    guard animated else {
      ...
      textView.attributedText = NSAttributedString(string: item.description,
                                                   attributes: attributes())
      //Color
      imageView.tintColor = survey.isComplete ? Colors.getColor(forId: item.order) : isChosen ? survey.topic.tagColor : .systemGray
      imageView.setImage(image)
      if survey.isComplete {
        stackView.backgroundColor = !isChosen ? .clear : self.traitCollection.userInterfaceStyle == .dark ? .tertiarySystemBackground : Colors.getColor(forId: item.order).withAlphaComponent(0.15)
        setVoters()
      } else {
        stackView.backgroundColor = !isChosen ? .clear : self.traitCollection.userInterfaceStyle == .dark ? .tertiarySystemBackground : survey.topic.tagColor.withAlphaComponent(0.15)
      }
      return
    }
  ...
  func setVoters() {
    guard item.totalVotes != 0 else {
      ...
      votersCountLabel.text = "no_votes".localized.uppercased()
      votersCountLabel.textColor = .secondaryLabel
      
      return
    }
    
    votersCountLabel.textColor = .label
    disclosureIndicator.tintColor = Colors.getColor(forId: item.order)
    ...
    voters.enumerated().forEach { index, userprofile in
      let avatar = Avatar(isBordered: true,
                          borderColor: isChosen ? traitCollection.userInterfaceStyle == .dark ? .tertiarySystemBackground : Colors.getColor(forId: item.order) : .clear)
  ...
  func appendVoter(userprofile: Userprofile) {
    ...
    if avatars.isEmpty, let imageView = horizontalStack.getSubview(type: UIImageView.self, identifier: "chevron"), let widthConstraint = votersView.getConstraint(identifier: "width") {
      ...
      UIView.transition(with: votersCountLabel, duration: 0.5, options: .transitionCrossDissolve) {
        ...
        self.votersCountLabel.textColor = .label
