...
//  public private(set) var compatibility: TopicCompatibility?
  ...
  private var barMode = Enums.SearchMode.off {
    didSet {
      guard oldValue != barMode else { return }
      
      onBarModeChanged()
      
      guard barMode == .on else { return }
...
extension SurveysController: SurveysViewInput {
  func getDataItems(excludeList: [SurveyReference]) {
    controllerInput?.getDataItems(filter: filter, excludeList: excludeList, substring: "")
  }
...
//        self.controllerInput?.getDataItems(filter: filter, excludeList: <#T##[SurveyReference]#>, owners: <#T##[Userprofile]#>, substring: $0)
      }
      .store(in: &subscriptions)
  }
...
//        controllerOutput?.toggleSearchMode(true)
    ...
    searchString = barMode == .on ? searchString: ""
    toggleSearchField(on: barMode == .on ? true : false)
    toggleTopicView(on: barMode != .on ? true : false)
  }
...
//        else { return }
//
//        self.controllerInput?.getDataItems(filter: <#T##SurveyFilter#>, excludeList: <#T##[SurveyReference]#>, owners: <#T##[Userprofile]#>, substring: <#T##String#>)
