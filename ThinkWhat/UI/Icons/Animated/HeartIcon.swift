...
class HeartIcon: Icon, CAAnimationDelegate {
    ...
    func setupLayerFrames(){
        ...
        if let path = layers["path"] as? CAShapeLayer{
            path.frame = CGRect(x: 0.11111 * path.superlayer!.bounds.width, y: 0.16695 * path.superlayer!.bounds.height, width: 0.77777 * path.superlayer!.bounds.width, height: 0.6661 * path.superlayer!.bounds.height)
    ...
    func addDisableAnimation(completionBlock: ((_ finished: Bool) -> Void)? = nil){
        ...
        let pathDisableAnim : CAAnimationGroup = QCMethod.group(animations: [pathFillColorAnim, pathTransformAnim], fillMode:fillMode)
